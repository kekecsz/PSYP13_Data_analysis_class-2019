# ---	
# title: "Exercise 14 - Sepcial Predictors"	
...
# The result of this test tell us that the mean post-treatment BMI is significantly different among groups.	
...
# The coefficient of the intercept is a contant (different for each regression model) that is not dependent on the values of the predictors. It can be interpreted as if all the predictors in the model would have the value of zero (0), this would be the estimated value for the outcome.	
...
# Now we can use the same interpretation that we are used to to the regression coefficients in the coefficient table: The intercept is the value of the outcome if every predictor's value is zero (this basically means that this is the predicted value of the outcome variable at the "default level". The coefficients of the predictors show the amount of change in the outcome variable's value if the given predictor's value increases by 1 point. This can happen for "got_pill" if the person got the treatment "pill". This change is always in relation to the default level.	
...
# ## Introducinginteraction terms into the model	
...
# We canenter the interaction term into the model by using a * isntead of a + between the predictors we want to include the interaction of.	
