# ---	
# title: "Exercise 16 - Model diagnostics"	
...
# More problematic are cases that not only have very different values from the other data, but they have a high influence on the regression line as well. Highly influential cases can be identified by looking at the scatterplot with the regression line, by plotting the residual - levarage plot, and by using Cook's distance.	
...
# We can check this assumption by looking at the plot of the standardized residuals and the predicted values, where we should see roughly equal variation at all values of the predicted values. There are also good statistical tests to check this assumption. The Breush-Pagan test is initiated with the bptest() function from the lmtest package, and the NCV test is performed by the ncvTest() funtion from base R (no package needed). A p-value < 0.05 in these tests would indicate a violation of the assumption of homoscedasticity, so it would mean that there is significant heteroscedasticity. 	
