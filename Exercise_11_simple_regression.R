# ---	
# title: "Exercise 11 - Linear regression, basic concepts"	
...
# There are several irregularties in the dataset, such as the height is a factor, and has some weird values, and also the variable 'gender' has inconsistency in coding. We need to handle these before going forward.	
...
mydata_cleaned <- mydata %>% 	
  mutate(	
         height = as.numeric(as.character(replace(height, height == "168cm", 168))),	
         height = as.numeric(as.character(replace(height, height == "64", 164))),	
         gender = droplevels(replace(gender, gender == "Female", "female")),	
         gender = droplevels(replace(gender, gender == "Male", "male"))	
         ) 	
# if the variable gender is identified as a factor, we can drop the factor levels that no longer use (such as "Female" and "Male" instead of "female" and "male") using the droplevels() function	
...
# shoe size = intercept + heights_coefficient * height	
...
# This means that by using the regression model, we are able to explain ... of the variability in the outcome.	
